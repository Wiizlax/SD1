...
public class Consigne{
	...
	public int attribuerCasierLibre(String motDePasse) {
		...
		if (casiersLibres.estVide()) return -1;
		Casier casierLibre = casiersLibres.sommet();
		int indiceCasier = casierLibre.getNumero();
		casiersLibres.pop();
		tousLesCasiers[indiceCasier].setMotDePasse(motDePasse);
		return indiceCasier;
	}
	...
	public boolean libererCasier(int numeroCasier, String motDePasse) {
		...
		if ((numeroCasier>=0 && numeroCasier<tousLesCasiers.length) && motDePasse.equals(tousLesCasiers[numeroCasier].getMotDePasse())){
