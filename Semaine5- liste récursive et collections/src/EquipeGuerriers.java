...
public class EquipeGuerriers {
	...
	public Guerrier jouer(int pointsDeViePerdus){
		...
		Guerrier combattant = listeGuerriersEnJeu.getFirst();
		listeGuerriersEnJeu.remove(combattant);
		combattant.setPointsDeVie(combattant.getPointsDeVie()-pointsDeViePerdus);
		if (combattant.getPointsDeVie()>0){
			listeGuerriersEnJeu.addLast(combattant);
		}
		return combattant;
	}
	...
	public int ajouterNouveauGuerrier() {
		...
		for (Guerrier guerrier1 : listeGuerriersEnJeu) {
			if (guerrier1.getNumero() > max) {
				max = guerrier1.getNumero();
				pos = i;
			}
			i++;
		}
		listeGuerriersEnJeu.add(pos+1,guerrier);
		return numG;
	}
